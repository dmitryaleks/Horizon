...
from horizon.simulation import (
    _compute_ratios,
    _compute_weights,
    _bootstrap_sample,
...
class TestComputeWeights:
    def test_exact_match_gets_highest_weight(self):
        ...
        weights = _compute_weights(tasks, target_sp=5.0, sigma=2.5)
    ...
    def test_very_large_sigma_gives_near_uniform(self):
        tasks = make_diverse_tasks()
        weights = _compute_weights(tasks, target_sp=5.0, sigma=1000.0)
    ...
    def test_very_small_sigma_concentrates_on_closest(self):
        tasks = make_diverse_tasks()
        weights = _compute_weights(tasks, target_sp=5.0, sigma=0.01)
    ...
    def test_single_task_gets_weight_one(self):
        ...
        weights = _compute_weights(tasks, target_sp=8.0, sigma=2.5)
...
class TestBootstrapSample:
    def test_output_length(self):
        ...
        samples = _bootstrap_sample(ratios, weights, 5000, rng)
    ...
    def test_values_come_from_ratios(self):
        ...
        rng = np.random.default_rng(42)
        samples = _bootstrap_sample(ratios, weights, 1000, rng)
        assert set(samples).issubset({1.0, 2.0, 3.0})

    def test_deterministic_with_seed(self):
        ratios = np.array([1.0, 1.5, 2.0])
        weights = np.array([0.2, 0.5, 0.3])
        s1 = _bootstrap_sample(ratios, weights, 100, np.random.default_rng(42))
        s2 = _bootstrap_sample(ratios, weights, 100, np.random.default_rng(42))
...
class TestExtractPercentiles:
    def test_basic(self):
        ...
        pe = _extract_percentiles(samples)
